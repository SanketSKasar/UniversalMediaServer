...
//		}
	...
	private void appendThumbnail(StringBuilder sb, MediaType mediaType) {
		...
		switch (mediaType.MediaTypeInt) {
			case MediaType.AUDIO_INT:
				...
				if (imageInfo != null) {
					if (imageInfo.getWidth() > 160 || imageInfo.getHeight() > 160) {
					...
					if (imageInfo.getWidth() > 640 || imageInfo.getHeight() > 480) {
						addAlbumArt(sb, DLNAImageProfile.JPEG_MED);
						addImageResource(sb, DLNAImageProfile.JPEG_MED, imageInfo);
					}
					if (imageInfo.getWidth() > 1024 || imageInfo.getHeight() > 768) {
	...
	private void addImageResource(StringBuilder sb, DLNAImageProfile imageProfile, ImageInfo imageInfo) {
		...
		if (StringUtils.isNotBlank(thumbURL)) {
			...
			if (imageInfo == null) {
				ciFlag = "";
			} else if (imageInfo.getFormat().equals(ImageFormat.toImageFormat(imageProfile))){
				boolean conversionIndicator;
				switch (imageProfile.toInt()) {
					case DLNAImageProfile.GIF_LRG_INT:
						conversionIndicator = imageInfo.getWidth() > 1600 || imageInfo.getHeight() > 1200;
						break;
					case DLNAImageProfile.JPEG_LRG_INT:
						conversionIndicator = imageInfo.getWidth() > 4096 || imageInfo.getHeight() > 4096;
						break;
					case DLNAImageProfile.JPEG_MED_INT:
						conversionIndicator = imageInfo.getWidth() > 1024 || imageInfo.getHeight() > 768;
						break;
					case DLNAImageProfile.JPEG_RES_H_V_INT:
						conversionIndicator = false;
						break;
					case DLNAImageProfile.JPEG_SM_INT:
						conversionIndicator = imageInfo.getWidth() > 640 || imageInfo.getHeight() > 480;
						break;
					case DLNAImageProfile.JPEG_TN_INT:
						conversionIndicator = imageInfo.getWidth() > 160 || imageInfo.getHeight() > 160;
						break;
			...
			if (imageInfo != null) {
				if (imageInfo.getSize() > 0) {
					addAttribute(sb, "size", Long.toString(imageInfo.getSize()));
				}
				addAttribute(sb, "resolution", Integer.toString(imageInfo.getWidth()) + "x" + Integer.toString(imageInfo.getHeight()));
			}
	...
	public synchronized InputStream getInputStream(Range range, RendererConfiguration mediarenderer) throws IOException {
		...
		if (player == null && !isResume()) {
			...
			if (
				...
				//XXX The autorotate seems to work only for JPEG, this should be updated and the restriction below lifted
	...
	public final DLNAThumbnailInputStream getGenericThumbnailInputStream(String fallback) throws IOException {
		...
		thumbnailImageInfo = ImageInfo.toImageInfo(inputStream);
		return inputStream;
	}
