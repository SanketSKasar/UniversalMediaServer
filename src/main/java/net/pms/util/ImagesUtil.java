...
public class ImagesUtil {
	...
	protected static Image transcodeImage(
		...
		if (width == 0 ||
			...
			if (inputResult.imageFormat.equals(outputFormat)) {
				// Nothing to do, just return source
				if (dlnaThumbnail) {
					return new DLNAThumbnail(inputByteArray, outputFormat, bufferedImage, metadata, false);
				} else if (dlnaCompliant) {
					return new DLNAImage(inputByteArray, outputFormat, bufferedImage, metadata, false);
				} else {
		...
		if (dlnaThumbnail) {
			return new DLNAThumbnail(outputByteArray, outputFormat, bufferedImage, metadata, false);
		} else if (dlnaCompliant) {
			return new DLNAImage(outputByteArray, outputFormat, bufferedImage, metadata, false);
		} else {
